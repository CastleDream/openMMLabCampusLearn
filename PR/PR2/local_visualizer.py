...
class SegLocalVisualizer(Visualizer):
    ...
    def _draw_sem_seg(self, image: np.ndarray, sem_seg: PixelData,
        ...
        Args:
            ...
            palette (list, optional): Input palette for result rendering, which
                ...
                Defaults to None.
            withLabels(bool, optinal): Add semantic labels in visualization result, True by default.
        ...
        if withLabels:
            ...
            thickness = 3
            lineType = 2
            print(f"type(sem_seg[0]), sem_seg[0]")
            masks = sem_seg[0] == labels[:, None, None]
            print(f"type(masks), masks")
            for mask_num in range(len(labels)):
                ...
                mask = cv2.rectangle(
                    mask, loc, (loc[0]+label_width+baseline, loc[1]+label_height+baseline), classes_color, -1)
                mask = cv2.rectangle(
                    mask, loc, (loc[0]+label_width+baseline, loc[1]+label_height+baseline), (0, 0, 0), 2)
                mask = cv2.putText(
                    mask, text, (loc[0], loc[1]+label_height), font, fontScale, fontColor, thickness, lineType)
        color_seg = (image * (1 - self.alpha) +
                     mask * self.alpha).astype(np.uint8)
    ...
    def add_datasample(
        ...
        Args:
            ...
            withLabels(bool, optinal): Add semantic labels in visualization result, Defaults to True.
        """
